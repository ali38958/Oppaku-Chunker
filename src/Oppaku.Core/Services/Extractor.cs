...
public class Extractor
{
    ...
    public string ComputeSourceFileHash(string sourcePath)
    {
        ...
        _cachedSourceHash = ChecksumHelper.ComputeFileHash(sourcePath);
        _cachedSourcePath = sourcePath;
        
        return _cachedSourceHash;
    }

    public void ExtractChunk(string sourcePath, int chunkIndex, long chunkSize, string outputDir, string sourceFileHash)
    {
        if (!File.Exists(sourcePath))
            throw new OppakuException(ErrorCode.InvalidChunk, $"Source file not found: {sourcePath}");
        if (chunkIndex < 0)
        ...
        using (var handle = File.OpenHandle(sourcePath, FileMode.Open, FileAccess.Read, FileShare.Read))
        using (var outStream = new FileStream(chunkPath, FileMode.Create, FileAccess.Write, FileShare.None, 64 * 1024))
        using (var writer = new BinaryWriter(outStream, System.Text.Encoding.UTF8, leaveOpen: true))
        {
            ...
            using (var cryptoStream = new CryptoStream(outStream, sha256, CryptoStreamMode.Write, leaveOpen: true))
            {
                ...
                // Seek back to start and overwrite dummy header with real checksum
